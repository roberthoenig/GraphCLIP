...
def dict_to_pyg_graph(d, img_enc, txt_enc, image_id_to_path):
    ...
    for idx, obj in enumerate(d['objects']):
        ...
        # TODO: normalize?
        feature = txt_enc([obj['names'][0]])
        x_features.append(feature)
    x = torch.concat(x_features)
    # edge_index: [2, num_edges], edge_attr: [num_edges, num_txt_features]
    ...
    for ctr, rel in enumerate(d['relationships']):
        ...
        feature = txt_enc([rel['predicate']])
        edge_features.append(feature)
    if len(edge_features) == 0:
        edge_attr = torch.zeros(0, 1024)
    else:
        edge_attr = torch.concat(edge_features)
