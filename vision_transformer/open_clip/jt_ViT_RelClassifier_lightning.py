...
class ViT_RelClassifier(pl.LightningModule):
    def __init__(self, 
        ...
        if rel_occurence_probabilities is not None and obj_occurence_probabilities is not None and attr_occurence_probabilities is not None:
            ...
            print("WARNING: occurence probabilities are not registered yet.")



    def register_occurence_probabilities(self, rel_occurence_probabilities, obj_occurence_probabilities, attr_occurence_probabilities):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_attr = self.attr_criterion(attr, lattr)
        loss = loss_rel + loss_obj_1 + loss_obj_2 + 10*loss_attr
        ...
        self.log_dict({
            ...
            "attr_precision": self.attr_precision(attr, lattr),
            "attr_recall": self.attr_recall(attr, lattr),
            "attr_acc": self.attr_accuracy(attr, lattr),
            "epoch": float(self.current_epoch),
            "epoch_progress": batch_idx/self.dataloader_length
        }, on_step=True, on_epoch=True, prog_bar=True, logger=True, sync_dist=True)
        # Log the batch loss and accuracy to wandb
        # self.log("train_loss", loss.item(),)
        # self.log("rel_loss", loss_rel.item())
        # self.log("obj_1_loss", loss_obj_1.item())
        # self.log("obj_2_loss", loss_obj_2.item())
        ...
        # self.log("epoch_progress", batch_idx/self.dataloader_length)
        # return {
        #     "loss": loss, "rel": rel, "obj_1": obj_1, "obj_2": obj_2, "attr": attr,"lrels": lrels, "lobj1s": lobj1s, "lobj2s": lobj2s, "lattr": lattr, "batch_idx": batch_idx,
