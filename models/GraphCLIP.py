...
from utils.dataset_utils import MultiDataLoader, dataset_filter, make_sample_all_relations_batched, make_sample_relation_batched, make_swap_attributes_batched, transfer_attributes_batched, tokens_to_embeddings_batched
...
class GNN15(torch.nn.Module):
    ...
    def forward(self, data, exclude_from_dropout=None, return_dropout_mask=False, dropout_mask=None, p_dropout=None):
        data = tokens_to_embeddings_batched(data, self.embedding)
        x, edge_index, edge_attr, batch = data.x, data.edge_index, data.edge_attr, data.batch
        x += torch.randn(x.shape) * (self.noise if self.training else 0.0)
        edge_index, edge_mask, node_mask = dropout_node_keep_master_nodes(edge_index=edge_index,
                                                                  batch=batch, training=self.training,
                                                                  p=p_dropout, exclude_from_dropout=exclude_from_dropout,
                                                                  dropout_mask=dropout_mask)
    ...
    def train(self):
        ...
        def config_to_trainset_valset_batchsize_advtransform(cfg):
            ...
            if adv_transform == "swap_attrs":
                adv_transform = make_swap_attributes_batched(**cfg['train_args'].get('adv_transform_args', dict()))
